...
# function to calculate tp, fp and fn chapters
...
for _ in files:
    ...
    lang_emb = _file['language'][:-1]
    aud_emb = _file['audio']
    vid_emb = _file['video']
    query_emb = _file['language'][-1]

    text_scores = torch.softmax( lang_emb@query_emb.T, dim=-1).cpu().numpy()
    aud_scores = torch.softmax( aud_emb@query_emb.T, dim=-1).cpu().numpy()
    vid_scores = torch.softmax( vid_emb@query_emb.T, dim=-1).cpu().numpy()
    top_two_sub_indices = np.argsort(text_scores)[-2:][::-1]
    top_two_vid_indices = np.argsort(vid_scores)[-2:][::-1]
    top_two_aud_indices = np.argsort(aud_scores)[-2:][::-1]
    
    try:
        subtitles_retrieved_chaps = [_file["chapters"][top_two_sub_indices[0]], _file["chapters"][top_two_sub_indices[1]] ]
        video_retrieved_chaps = [_file["video_chapters"][top_two_vid_indices[0]], _file["video_chapters"][top_two_vid_indices[1]]]
        audio_retrieved_chaps = [_file["audio_chapters"][top_two_aud_indices[0]], _file["audio_chapters"][top_two_aud_indices[1]]]
...
print('samples of audio, video and subtitles processed: ',  audio_counter, video_counter, sub_counter)
print( 'errors in audio, video, subtitles: ',audio_lapses, video_lapses, sub_lapses)

print( 'number of files probed: ',files_probed)
print('\n')
